...
public class VariableLengthArmSim extends LinearSystemSim<N2, N1, N2> {
    ...
    public VariableLengthArmSim(
            ...
            double armMassKg,
            boolean simulateGravity) {
        super(Models.singleJointedArmFromPhysicalConstants(gearbox, jKgMetersSquared, gearing));
        m_gearbox = gearbox;
        m_gearing = gearing;
        m_moi = jKgMetersSquared;
        m_r = armLengthMeters / 2;
                ...
                (Matrix<N2, N1> x, Matrix<N1, N1> _u) -> {
                    ...
                    if (m_simulateGravity) {
                        xdot = xdot.plus(VecBuilder.fill(
                                0, m_armMass * m_r * -9.8 / (m_moi) * Math.sin(x.get(0, 0) - m_gravityAngle)));
                    }
                    return xdot;
                },
                currentXhat,
    ...
    private double getB11() {
        return m_gearing * m_gearbox.Kt / (m_gearbox.R * m_moi);
    }
        ...
        m_plant.getA()
                .set(
                        ...
                        -m_gearing
                                * m_gearing
                                * m_gearbox.Kt
                                / (m_gearbox.Kv * m_gearbox.R * m_moi));
        m_plant.getB().set(1, 0, m_gearing * m_gearbox.Kt / (m_gearbox.R * m_moi));
    }
